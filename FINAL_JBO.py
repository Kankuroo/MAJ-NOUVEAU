...
gérer des comptes JBO (travaux), d'enregistrer les différentes étapes d'un travail avec
...
class DataManager:
    """Gestionnaire de base de données pour les comptes JBO et leurs étapes."""
    ...
    def generate_new_ref(self) -> str:
        """Génère une nouvelle référence JBO en incrémentant le plus grand numéro existant."""
        conn = self._connect()
        cur = conn.cursor()
        cur.execute("SELECT ref FROM jobs WHERE ref LIKE 'JBO %' ORDER BY ref DESC LIMIT 1")
        row = cur.fetchone()
        conn.close()
...
class JobWindow(tk.Toplevel):
    """Fenêtre de détail d'un job JBO."""
...
class MainApplication(tk.Tk):
    ...
    def __init__(self, data_manager):
        ...
        add_btn = ttk.Button(toolbar, text="Nouveau JBO", command=self.new_job)
        ...
        edit_btn = ttk.Button(toolbar, text="Modifier JBO", command=self.edit_job)
    ...
    def export_excel(self):
        ...
        file_path = filedialog.asksaveasfilename(
            ...
            title="Exporter les JBO terminés"
        )
        if file_path:
            try:
...
class ArchivedJobsWindow(tk.Toplevel):
    ...
    def __init__(self, master, data_manager: DataManager, title: str = "Archives JBO"):
        super().__init__(master)
        self.data_manager = data_manager
        self.title(title)
        self.geometry("900x600")
        ...
        toolbar = ttk.Frame(self)
        toolbar.pack(fill='x', pady=5)
        ttk.Button(toolbar, text="Nouveau JBO", state='disabled').pack(side='left', padx=5)
        ttk.Button(toolbar, text="Supprimer", state='disabled').pack(side='left', padx=5)
        ttk.Button(toolbar, text="Modifier JBO", state='disabled').pack(side='left', padx=5)
        refresh_btn = ttk.Button(toolbar, text="Actualiser", command=self.refresh_job_list)
        refresh_btn.pack(side='left', padx=5)
